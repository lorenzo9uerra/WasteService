...
public class TestDeposit {
    ...
    final static String CTX_TEST = "ctxreq_deposit";

    String test_prefix = "dep_";
    String actor_trolley = test_prefix + "trolley";

    TrolleyPosObserver trolleyPosObserver;

    // rappresenta GLASS_BOX e PLASTIC_BOX in questo test,
    // non effettiva modellazione
    String actor_boxes = test_prefix + "waste_boxes";
    ...
    public void testDeposit() {
        String wasteBoxResponse = coapRequest(actor_boxes);
        ...
        for (int i = 0; i < maxSecondsWait; i++) {
            ...
            if (lastPos.equals("glass_box")) {
                ...
                CommUtils.delay(200);
                wasteBoxResponse = coapRequest(actor_boxes);
    ...
    protected void startDeposit(String type, int amount) {
        String startDepositDispatch = MsgUtil.buildDispatch("_", "deposit",
                "deposit(" + type + ", " + amount + ")",
                actor_trolley
        ).toString();
        try {
