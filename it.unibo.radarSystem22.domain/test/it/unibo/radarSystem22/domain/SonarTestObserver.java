...
public class SonarTestObserver implements ISonarObserver, IDistanceObserver {
    private ISonarObservable sonar;
    private IDistanceObservable distanceObs;
    ...
    public SonarTestObserver(ISonarObservable sonar, IDistanceObservable distanceObs, int maxDelta) {
        this.sonar = sonar;
    ...
    public synchronized void update(IDistance distance) {
        ...
        // assertTrue in sub-thread lancia eccezione senza far fallire test, passa a thread principale
            ...
            System.out.println("[Sonar test observer] excessive change " + (curVal - prevValue) + ", failing");
            success = false;
            assertErr = e;
            sonar.unsubscribe(this);
            distanceObs.unsubscribe(this);
        }
